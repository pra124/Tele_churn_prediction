...
def xgb_algo(X_train, y_train, X_test, y_test):
    try:
        ...
        logger.info(f'Test Accuracy XGBoost : {accuracy_score(y_test, xgb_model.predict(X_test))}')
        logger.info(f'Confusion Matrix : {confusion_matrix(y_test, xgb_model.predict(X_test))}')
        logger.info(f'Classification Report : {classification_report(y_test, xgb_model.predict(X_test))}')
...
def best_model_using_acu_roc(X_train,y_train,X_test,y_test):
    try:
        ...
        xgb_fpr, xgb_tpr,xgb_thre = roc_curve(y_test, rf_pred)
        svm_fpr,svm_tpr,svm_thre= roc_curve(y_test, rf_pred)
        plt.xlabel("FPR")
        plt.ylabel("TPR")
        ...
        plt.plot(xgb_fpr, xgb_tpr, color='orange', label=f"XGB Algo {round(auc(xgb_fpr, xgb_tpr), 3)}")
