...
from filter_methods import chi_square, anova,t_test,correlatation
...
class CHURN:
    ...
    def filter(self):
        try:
            ...
            c=correlatation(self.x_train_num1,self.y_train)
        except Exception as e:
            e_type, e_msg, e_linno = sys.exc_info()
            logger.info(f'Issue is:{e_linno.tb_lineno} due to {e_msg}')
    ...
    def parameters(self):
        try:
            ...
            #reg1 = LogisticRegression(C=1.0,class_weight= None,l1_ratio= None, max_iter=100, multi_class='ovr', n_jobs= None, penalty='l1', solver= 'liblinear')
            #reg1.fit(self.training_data_res_t, self.y_train_res)
...
if __name__=='__main__':
    try:
        path='C:\\Users\\prana\\Downloads\\telephn_churn\\WA_Fn-UseC_-Telco-Customer-Churn (3).csv'
        obj=CHURN(path)
        obj.knn()
