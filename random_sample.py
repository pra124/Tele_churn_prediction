...
def random_sample_imputation(x_train, x_test, column):
    try:
        ...
        if x_train[column].isnull().sum() > 0:
            train_random = (x_train[column].dropna().sample(x_train[column].isnull().sum(), replace=True, random_state=24))
        ...
        if x_test[column].isnull().sum() > 0:
            ...
            test_random = source.sample(x_test[column].isnull().sum(), replace=True, random_state=24)
